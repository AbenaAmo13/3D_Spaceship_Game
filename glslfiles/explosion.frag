...
in  vec2 exiting_TexCoord; //texture coord arriving from the vertex
...
void main(void)
{
	//out_Color = texture(DiffuseMap, exiting_TexCoord); //show texture values

	//out_Color = vec4(ex_Normal,1.0); //Show normals

	//out_Color = vec4(exiting_TexCoord,0.0,1.0); //show texture coords
	...
	//out_Color = texture(DiffuseMap, exiting_TexCoord); //show texture only
    out_Color = color * texture(DiffuseMap, exiting_TexCoord); //show texture and lighting
}